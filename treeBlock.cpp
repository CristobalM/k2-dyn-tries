...
//       if (lowerB <= subtrees[i].subtreeSize && subtrees[i].subtreeSize <= upperB && subtrees[i].preorder < leftmost) {
    ...
    if (((float) nNodes / 4) <= subtrees[i].subtreeSize && subtrees[i].subtreeSize <= ((float) 3 * nNodes / 4) &&
        subtrees[i].preorder < leftmost) {
...
void treeBlock::insert(TreeInfo *treeInfo, treeNode node, uint8_t str[], uint64_t length, uint16_t level,
  ...
  if (ptr != NULL && curFlag < nPtrs && absolutePosition(node) == ((blockPtr *) ptr)[curFlag].flag) {
    ...
    dfuds[node.first] = dfuds[node.first] | (insertT[cNodeCod][str[0]] << aux);
    return;
  } else if ((int) nNodes + (int) length - 1 <= maxNodes) {
    ...
    while (preorderOrigNode >= preorderNode) {
      ...
      dfuds[aux2/*destNode.first*/] = dfuds[aux2/*destNode.first*/]
                                      | (((dfuds[preorderOrigNode >> 2/*origNode.first*/]
              >> 4 * (3 - (preorderOrigNode & 0x3)/*origNode.second*/))
                                          & 0x000F) << aux);
      //prevNode(destNode);
      --preorderDestNode;
      //prevNode(origNode);
...
uint64_t sizeTrie(trieNode *t) {
  ...
  uint64_t totalSize = /*sizeof(void *) +*/4 *
                                           sizeof(uint16_t); // si usamos hasta 8 niveles de punteros, los punteros a hijo del trie se pueden implementar con 16 bits
